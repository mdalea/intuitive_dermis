...
for h=1:hem_cnt
    ...
    mkdir 'global_outfile_tbcas24/spikegen'
    tex_name = stim.tex_name;  writecell(tex_name,[current_loc,'global_outfile_tbcas24/spikegen/tex_name.txt']);    
...
% lowest noise removed - highest noise 10x higher
noiseVar = [0 0 0 0 0; 
    0 0 1e-3^2 0 0; 0 0 10e-3^2 0 0; 0 0 100e-3^2 0 0; 
    0 0 0 10e-3^2 0; 0 0 0 100e-3^2 0; 0 0 0 500e-3^2 0;  
    0 0 0 0 1e6*((1e-3)^2/log(10e3/1)); 0 0 0 0 1e6*((10e-3)^2/log(10e3/1)); 0 0 0 0 1e6*((100e-3)^2/log(10e3/1)); % convert noise power density at 1Hz to noise power over band
    0 1e-3^2 0 0 0; 0 10e-3^2 0 0 0; 0 100e-3^2 0 0 0;
    0 10e-3^2 10e-3^2 100e-3^2 1;
    0 100e-3^2 100e-3^2 500e-3^2 10;]; % noise + th mismatch + th noise
    ...
    for N_th=N_th_array
        ...
        for tt=1:size(trials_for_train,1)
            ...
            for exp_tf = 0
                ...
                folderPath = [current_loc,'global_outfile_tbcas24/spikegen/ALL_N_lcadc-',num2str(1e6*noiseVar(i,1)),'-uV2-',num2str(1e6*noiseVar(i,2)),'-uV2-',num2str(1e6*noiseVar(i,3)),'-uV2-',num2str(noiseVar(i,5)),'-mult-',num2str(1e6*noiseVar(i,4)),'-uV2-',num2str(N_th)];
                folderPath_mrindex = [current_loc,'global_outfile_tbcas24/spikegen/mr_index'];
