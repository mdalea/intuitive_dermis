...
LSB     = 500e-3;          % quantization step [V] (spike step / ADC LSB)
...
% ---- Auto-zero settings ----
useUniformAZ     = true;   % uniform-sampling AZ
Fsample_Hz       = 1e3;    % uniform sampling + AZ rate [Hz]
useEventAZ       = true;   % event-triggered AZ on LC path
%% ----------------------------------------------------------------------
...
% C1) UNIFORM sampling, NO AZ, ZOH back to Fs grid, WITH quantization
%     (same LSB as LC)
if useUniformAZ
    y_rec_uNoAZ = uniform_sample_ZOH_noAZ(x0, nth, nfk, Fs, Fsample_Hz, LSB);
end

% C2) UNIFORM sampling, WITH AZ, ZOH back to Fs grid, WITH quantization
if useUniformAZ
    y_rec_uAZ   = uniform_sample_autozero_ZOH(x0, nth, nfk, Fs, Fsample_Hz, LSB);
end
...
if useUniformAZ
    [fU0, PSDU0_dBFSbin, MU0] = your_welch_psd_and_metrics(y_rec_uNoAZ, Fs, fin, ...
        windowSize, overlap, FS_Power, flo, fbw, harmonics, windowTime);

    [fU1, PSDU1_dBFSbin, MU1] = your_welch_psd_and_metrics(y_rec_uAZ, Fs, fin, ...
        windowSize, overlap, FS_Power, flo, fbw, harmonics, windowTime);
end
...
if useUniformAZ
    fprintf('\n=== Metrics (Uniform sample, NO AZ @ %g Hz) ===\n', Fsample_Hz);
    fprintf('SNR   : %6.2f dB\n',  MU0.SNR_dB);
    fprintf('SNDR  : %6.2f dB\n',  MU0.SNDR_dB);
    fprintf('SFDR  : %6.2f dB\n',  MU0.SFDR_dB);
    fprintf('ENOB  : %6.3f bits\n',MU0.ENOB_bits);
...
if useUniformAZ
    fprintf('Uniform path: sampled @ %.2f Hz\n', Fsample_Hz);
end

%% ------------------------------ Plots ----------------------------------
% Time-domain: input + selected reconstructions
tview = t < Tsec;
...
hold on; grid on;
if useUniformAZ
    plot(t(tview), y_rec_uNoAZ(tview), 'LineWidth',1);
    plot(t(tview), y_rec_uAZ(tview),  'LineWidth',1);
end
if useEventAZ
    plot(t(tview), y_rec_eAZ(tview), 'LineWidth',1);
end
xlabel('Time [s]'); ylabel('\itŷ\rm [V]');
lg = {};
if useUniformAZ
    lg{end+1} = sprintf('Uniform, NO AZ @ %g Hz',Fsample_Hz);
    lg{end+1} = sprintf('Uniform + AZ @ %g Hz',Fsample_Hz);
end
if useEventAZ,   lg{end+1} = 'LC: Event AZ'; end
legend(lg,'Location','best'); title('Reconstructions with/without autozero');
...
if useEventAZ
    ...
    leg{end+1} = 'LC: Event AZ';
end
if useUniformAZ
    kU0 = find(fU0>0,1,'first');
    kU1 = find(fU1>0,1,'first');
    semilogx(fU0(kU0:end), PSDU0_dBFSbin(kU0:end), 'LineWidth',1);
    semilogx(fU1(kU1:end), PSDU1_dBFSbin(kU1:end), 'LineWidth',1);
    leg{end+1} = sprintf('Uniform, NO AZ @ %g Hz',Fsample_Hz);
    leg{end+1} = sprintf('Uniform + AZ @ %g Hz',Fsample_Hz);
end
xlabel('Frequency [Hz]');
ylabel('PSD [dBFS]');
...
%% -------- Bar plot comparing SNR/SNDR/SFDR (no ENOB) --------
...
% Uniform, NO AZ
if useUniformAZ
    Y = [Y, [MU0.SNR_dB; MU0.SNDR_dB; MU0.SFDR_dB]];
    caseLabels{end+1} = sprintf('Uniform, NO AZ @ %g Hz', Fsample_Hz);

    Y = [Y, [MU1.SNR_dB; MU1.SNDR_dB; MU1.SFDR_dB]];
...
function y = uniform_sample_ZOH_noAZ(x0, nth, nfk, Fs, Fsample_Hz, LSB)
% Uniform sampling WITHOUT autozero; with amplitude quantization and ZOH.
    N  = numel(x0);
    ...
    for k = 1:numel(idx)
        n    = idx(k);
        % no AZ: keep flicker
        x_s  = x0(n) + nth(n) + nfk(n);

        % quantize with same LSB
        x_s_q = LSB * round(x_s / LSB);

        n1   = n;
        n2   = min(N, n + Ns - 1);
        y(n1:n2) = x_s_q;                    % hold quantized value
    end
end

function y = uniform_sample_autozero_ZOH(x0, nth, nfk, Fs, Fsample_Hz, LSB)
% Uniform sampling WITH ideal autozero of flicker; with amplitude quantization
% and ZOH back to Fs grid.
    N  = numel(x0);
    Ns = max(1, round(Fs / Fsample_Hz));     % samples per hold at Fs
    idx = 1:Ns:N;                            % sample indices on Fs grid
    y   = zeros(N,1);
