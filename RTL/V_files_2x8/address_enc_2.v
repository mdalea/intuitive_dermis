...
module address_enc_2(
	input [1:0] ao,   // LSB = bottom (ROW) or right-most (COL) pixel
	output reg address
...
*/
	always@(*) begin
		if (ao[0] == 1'b1) address = 1'd0;
		else if (ao[1] == 1'b1) address = 1'd1;
		//else if (ao[2] == 1'b1) address <= 2'd2;		
		//else if (ao[3] == 1'b1) address = 4'd3;
		//else if (ao[4] == 1'b1) address = 4'd4;
		//else if (ao[5] == 1'b1) address = 4'd5;
		...
		//else if (ao[10] == 1'b1) address = 4'd10;
		//else if (ao[11] == 1'b1) address = 4'd11;
		else address = 1'd0;   // handle erroneous address encoding
	end
endmodule
