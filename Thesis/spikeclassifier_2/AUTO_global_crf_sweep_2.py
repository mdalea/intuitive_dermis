# ============================================
# top_level_sweep.py (FULL, MODIFIED)
# ============================================
...
# -----------------------------
# NEW SWEEPS FOR DATASET VARIANTS
# -----------------------------
# You asked: "where X is [4 8 16 64]" for folder names containing "...-crf-X-..."
crf_array = [32, 80, 128, 150]

# These 4 dataset variants must match your folder names EXACTLY:
# 1) ...-crf-X-global_crf-skewdist-...
# 2) ...-crf-X-global_crf-unidist-max8-...
# 3) ...-crf-X-local_crf-skewdist-...
# 4) ...-crf-X-local_crf-unidist-max8-...
dataset_variants = [
    ("global_crf", "skewdist"),
...
# Original base datasetID (everything up to "...-3")
datasetID_base = "lcadc-0-uV2-0-uV2-0-uV2-0-mult-0-uV2-3"

# IMPORTANT: your new datasets changed from --1to40-1to40 to --1to10-1to10
...
for mr_cnt_val in mr_cnt_array:
    ...
    if side * side != mr_cnt_int:
        print(
            f"[WARN] mr_cnt={mr_cnt_int} is not a perfect square (fine with fixed X=16 addressing).",
            flush=True,
        )
    ...
    # -------------------------------------------------------------------------
    # NEW: change the SNN and SVM paths to reflect the new dataset path:
    #
    # You provided these patterns (X in [4,8,16,64]):
    #
    # ALL_N_lcadc-0-uV2-0-uV2-0-uV2-0-mult-0-uV2-3-crf-X-global_crf-skewdist-multitrial-Kylberg_filt_6_scan_oversampled20x_actualdimscale--1to10-1to10-mr_cnt-196-spatialres-0.2_sorted
    #
    # ALL_N_lcadc-0-uV2-0-uV2-0-uV2-0-mult-0-uV2-3-crf-X-global_crf-unidist-max8-multitrial-Kylberg_filt_6_scan_oversampled20x_actualdimscale--1to10-1to10-mr_cnt-196-spatialres-0.2_sorted
    #
    # ALL_N_lcadc-0-uV2-0-uV2-0-uV2-0-mult-0-uV2-3-crf-X-local_crf-skewdist-multitrial-Kylberg_filt_6_scan_oversampled20x_actualdimscale--1to10-1to10-mr_cnt-196-spatialres-0.2_sorted
    #
    # ALL_N_lcadc-0-uV2-0-uV2-0-uV2-0-mult-0-uV2-3-crf-X-local_crf-unidist-max8-multitrial-Kylberg_filt_6_scan_oversampled20x_actualdimscale--1to10-1to10-mr_cnt-196-spatialres-0.2_sorted
    #
    # IMPORTANT CHANGE:
    # We now pass --outdir_prefix to shorten the OUTPUT FOLDER NAME:
    #   savePath = <cwd>/SNN_dsetPath-<datasetFolderBasename>
    # No more "--shorten..." args.
    # -------------------------------------------------------------------------
    ...
    for crf_x in crf_array:
        for crf_scope, dist_tag in dataset_variants:
            ...
            for spatialres_val in spatialres_array:
                ...
                # EXACT dataset folder name (no trailing slash here)
                datasetFolderName = (
                ...
                run_cmd_or_die(
                    ...
                    f"--outdir_prefix SNN_dsetPath"
                )

                # ---------------- SVM (per fold) ----------------
                for fold in range(kfolds):
                    # With the new SNN naming, the checkpoint is inside:
                    #   <cwd>/SNN_dsetPath-<datasetFolderName>/chip-classifier-test-<fold>-best.pt
                    retrainPath = (
                        f"SNN_dsetPath-{datasetFolderName}/"
