...
% Normalized vs spatialres = 0.4 (Merkel cell pitch) + also plot UNnormalized
% All plots include a red dashed reference line labelled "Merkel cell pitch"
...
%% ---------------- Plot helpers ----------------
% Add Merkel cell pitch reference line:
% - vertical at x = 0.4 on all plots
% - horizontal at y = 1 on normalized plots
add_merkel_vertical = @() xline(ref_sr,'r--','Merkel cell pitch', ...
    'LabelOrientation','horizontal','LabelVerticalAlignment','bottom');
add_merkel_horizontal_norm = @() yline(1,'r--','Merkel cell pitch', ...
    'LabelOrientation','horizontal','LabelVerticalAlignment','bottom');

%% ---------------- Plot ONLY existing points (SEPARATE FIGURES) ----------------

% =========================
% Figure 1: SNN (UNnormalized)
% =========================
...
% Merkel cell pitch reference (vertical at x=0.4)
add_merkel_vertical();
...
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SNN-raw-%d.png', figi));
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SNN-raw-%d.fig', figi));

% =========================
% Figure 2: SVM (UNnormalized)
% =========================
...
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SVM-raw-%d.png', figi+1));
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SVM-raw-%d.fig', figi+1));

% =========================
% Figure 3: SNN (normalized)
% =========================
...
for im = 1:numel(mr_cnt_array)
    ...
    if nnz(mask) >= 1
        errorbar(x(mask), y(mask), e(mask), '-o', 'LineWidth', 2);
        legSNNn{end+1} = sprintf('mr\\_cnt=%d', mr_cnt_array(im)); %#ok<AGROW>
    end
end
...
% Merkel cell pitch references
add_merkel_vertical();        % x=0.4
add_merkel_horizontal_norm(); % y=1 baseline

if ~isempty(legSNNn)
    legend(legSNNn, 'Location','best');
else
...
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SNN-normMerkelPitch%.3g-%d.png', spatialres_array(ref_idx), figi+2));
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SNN-normMerkelPitch%.3g-%d.fig', spatialres_array(ref_idx), figi+2));

% =========================
% Figure 4: SVM (normalized)
% =========================
...
for im = 1:numel(mr_cnt_array)
    ...
    if nnz(mask) >= 1
        errorbar(x(mask), y(mask), e(mask), '-o', 'LineWidth', 2);
        legSVMn{end+1} = sprintf('mr\\_cnt=%d', mr_cnt_array(im)); %#ok<AGROW>
    end
end
...
if ~isempty(legSVMn)
    legend(legSVMn, 'Location','best');
else
    text(0.5,0.5,'No SVM results found (or missing 0.4 ref)','Units','normalized','HorizontalAlignment','center');
end

set(gcf,'Position',[800 600 650 450]);
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SVM-normMerkelPitch%.3g-%d.png', spatialres_array(ref_idx), figi+3));
saveas(gcf, sprintf('mrCnt_spatialRes_sweep-SVM-normMerkelPitch%.3g-%d.fig', spatialres_array(ref_idx), figi+3));
...
% advance figure counter by 4 since we used 4 figures
figi = figi + 4;
