# Texture 
#CUDA_VISIBLE_DEVICES=3 python intuitivespikes_snn_sg.py --datasetPath ../spikegen/global_outfile/spikegen/ALL_N_lcadc-0-uV-v2-0-uV-0-uV-0-uV-0-pct-4-mr_cnt-192-multitrial-Kylberg_filt_6_scan_actualdimscale_chip_lchpf--1to40-1to40_sorted/ --epochs 150 --hidden_size 256 --num_classes 6 --batch_size 32 --tmax_ms 400 --Ts 10 --kfolds 6 --cache yes --save_bestmodel yes
...
class SpkDataset_toFrameTimeBins(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Debug: print ALL x/y addresses for every sample handled + max x/y
        #if index == 0:
        #print("sample index:", index, "file id:", int(inputIndex.item()), "num events:", int(events.shape[0]))
        #print("x:", events["x"])
        #print("y:", events["y"])
        #print("max x:", int(events["x"].max()), "max y:", int(events["y"].max()))
        ## Optional: print as (x,y) pairs
        #print("xy pairs:", np.stack([events["x"], events["y"]], axis=1))
