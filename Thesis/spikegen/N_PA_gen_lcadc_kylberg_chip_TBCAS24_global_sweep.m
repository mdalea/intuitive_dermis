...
% ---- CHANGE: keep "requested" distribution separate; actual mr_in_hem will be set after clamping ----
if random_mr==1
    mr_in_hem_req = ones(1,hem_cnt);
else
    if crf==1
        if global_crf==1 && unidist==1
            mr_in_hem_req = 8.*ones(1,hem_cnt);   % requested
            %%mr_in_hem = 16.*ones(1,hem_cnt);   %% COMMENT THIS!!!
        elseif global_crf==1 && unidist == 0
            mr_in_hem_req = [8 5 3 1];  % requested per quadrant
...
if crf==1
    if global_crf==1
        ...
        % ---- CHANGE: make quadrant coverage GENERIC for any square mr_cnt (e.g., 4x4, 8x8, 16x16...) ----
        mr_side = sqrt(mr_cnt);
        if abs(mr_side - round(mr_side)) > 1e-12
            error('global_crf=0 expects mr_cnt to be a perfect square. mr_cnt=%d', mr_cnt);
        end
        mr_side = round(mr_side);
        if mod(mr_side,2) ~= 0
            error('global_crf=0 expects even side length. side=%d (mr_cnt=%d)', mr_side, mr_cnt);
        end
        q_side = mr_side/2;

        idxMat = reshape(1:mr_cnt, mr_side, mr_side); % column-major mapping
        % Match your original quadrant starts [1 5 33 37] for 8x8:
        % QI=top-left, QII=bottom-left, QIII=top-right, QIV=bottom-right
        QI   = idxMat(1:q_side,        1:q_side);
        QII  = idxMat(q_side+1:end,    1:q_side);
        QIII = idxMat(1:q_side,        q_side+1:end);
        QIV  = idxMat(q_side+1:end,    q_side+1:end);
    ...
    % ---- CHANGE: build ACTUAL mr_in_hem per heminode after clamping ----
    mr_in_hem = zeros(1,hem_cnt);

    for h=1:hem_cnt
        if global_crf == 1
            pool = hem_coverage(h,:);                 % all MRs
        else
            pool = hem_coverage(hem_order(h),:);      % quadrant pool
        end

        idx = randperm(numel(pool));                  % permute available pool
        ...
        % requested count
        if unidist == 1
            n_req = mr_in_hem_req(h);                 % per-heminode request (length hem_cnt)
        else
            n_req = mr_in_hem_req(hem_order_2(h));    % skew request (length 4)
        end

        % ---- CHANGE: clamp to available pool size (fixes your "Index exceeds 4" for mr_cnt=16 local mode) ----
        n_pick = min(n_req, numel(idx));

        mr_index{h} = pool(idx(1:n_pick));
        mr_in_hem(h) = n_pick;                        % store ACTUAL used count
    end
    cnt=hem_cnt;
else
    cnt=mr_cnt;
            ...
            for exp_tf = 0
                ...
                if crf==1
                    if read_mr_index == 1 || cr~=1   % always assume reusing MR indices when testing spatial compression rate
                        ...
                        % ---- CHANGE: keep mr_in_hem consistent with what is actually in mr_index ----
                        mr_in_hem = cellfun(@numel, mr_index);
                    else
                        ...
                        % ---- CHANGE: keep mr_in_hem consistent after pruning ----
                        mr_in_hem = cellfun(@numel, mr_index);
                    end
                end
