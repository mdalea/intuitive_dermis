...
# *********************************************************
# Meas
# USAGE: python.exe keysight_set_siggen.py 
#
# Hardware setup:
#   - DERMIS testboard
...
def init_siggen(instr):
    ...
    do_command_other(instr,"OUTP:LOAD 50") #high-Z
    time.sleep(2) 
    
def setup_siggen(instr, fin, vamp, vofs):
    do_command_other(instr,"FUNC SIN") #sine wave
    #do_command_other(instr,"FUNC RAMP") #sine wave
    time.sleep(2)
...
vamp=800e-3 #100x attenuator
fclk=0.1 #160 #fbw=250Hz, non-50Hz mulitple, 2nd tone outside band
vofs=0 #1.5

for run_id in range(0,int(1)): # perform N runs for averaging

    print("---------------------")
    print(f"Run # {run_id}\n")

    setup_siggen(RigolDG3061A, fclk, vamp, vofs) 
