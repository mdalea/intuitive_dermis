...
% Define input values
no_of_channels = [16, 70, 70, 19, 10, 4, 16, 16];
adc_bits = [24, 12, 12, 12, 14, 4, 6, 6];
sampling_rate = [300, 30, 30, 4.4e3, 390, 2*50, 2*500, 2*500];
...
power = 1e6 * [42e-3, 5*10e-3, 5*10e-3, 5*5e-3, 5*258e-6+3.3*100e-3, 10.6e-6*4+0.6e-3*3+2e-3*6, 24e-6*3*16+9e-3*6+0.6e-3*3, 24e-6*3*16]; % Power in µW
...
labels = {sprintf('[3]\nADS1258'), sprintf('[4]\nDSA9210'),  sprintf('[5]\nDSA9210'), ...
          sprintf('[17]\nPIC\\muC'), sprintf('[18]\nADPD1080\n+ Teensy 4.1'), ...
          sprintf('[9]\na-IGZO TFT'), sprintf('This Work\na-IGZO TFT\n'), sprintf('This Work\na-IGZO TFT\n(Taxel Array Only)')};
markers = {'p', 's', 'o', 'd', '^', 'v', 'v', 'v'}; % Marker types
...
for i = 1:length(raw_data_rate)
    ...
    elseif i == 6    
        text(log10(raw_data_rate(i)), log10(power(i)), labels{i}, 'VerticalAlignment', 'bottom', 'HorizontalAlignment', 'left');
    elseif i == 7    
        text(log10(raw_data_rate(i)), log10(power(i)), labels{i}, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left', 'Color', 'r');   
    elseif i == 8    
        text(log10(raw_data_rate(i)), log10(power(i)), labels{i}, 'VerticalAlignment', 'top', 'HorizontalAlignment', 'left', 'Color', 'r');           
    else
...
grid on;
xlabel('Raw Throughput = #channels \times ADC bits \times sampling rate');
